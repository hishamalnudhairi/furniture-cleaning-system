...
@section('content')
    ...
    <div class="mb-4 flex items-center justify-between gap-3">
        ...
        <a href="{{ route('admin.drivers.create') }}" class="btn btn-primary text-sm">+ {{ __('Add driver') }}</a>
    </div>
        ...
        <div class="mb-3 rounded-2xl bg-white p-4 shadow-sm ring-1 ring-slate-200">
            <div class="flex items-start justify-between gap-3">
                <div class="min-w-0">
                    ...
                    <p class="text-sm text-slate-500" dir="ltr">{{ $driver->phone }}</p>
                ...
                <div class="flex shrink-0 flex-col gap-2">
                    <a href="{{ route('admin.drivers.show', $driver) }}" class="rounded-lg bg-brand-600 px-4 py-2 text-center text-sm font-medium text-white hover:bg-brand-700">{{ __('View') }}</a>
                    <a href="{{ route('admin.drivers.edit', $driver) }}" class="rounded-lg bg-slate-100 px-4 py-2 text-center text-sm font-medium text-slate-700 hover:bg-slate-200">✎ {{ __('Edit') }}</a>
                </div>
            </div>
        </div>
    @empty
        <x-empty icon="🚚" :message="__('No drivers found.')" />
    @endforelse
