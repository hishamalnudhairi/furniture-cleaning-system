...
<body class="bg-slate-100 text-slate-800 antialiased">
    ...
    <div class="no-print mx-auto flex max-w-3xl items-center justify-between gap-2 px-4 py-4">
        ...
        <button onclick="window.print()" class="rounded-lg bg-brand-600 px-5 py-2 text-sm font-semibold text-white hover:bg-brand-700">🖨️ {{ __('Print') }}</button>
    </div>
    ...
    <div class="mx-auto max-w-3xl bg-white p-8 shadow-sm print:max-w-full print:p-0 print:shadow-none">
        ...
        <h2 class="mb-4 text-center text-lg font-bold tracking-wide text-brand-700">{{ __('Invoice') }}</h2>

        {{-- معلومات الطلب والعميل --}}
        <div class="mb-6 grid grid-cols-2 gap-4 text-sm">
            <div>
                <p class="mb-1 font-semibold text-slate-700">{{ __('Order') }}</p>
                <p>{{ __('Order number') }}: <span class="font-bold text-brand-700">{{ $order->order_number }}</span></p>
        ...
        <div class="flex items-start justify-between gap-6">
            ...
            <div class="w-64 space-y-1 text-sm">
                ...
                @endif
                <div class="flex justify-between border-t border-slate-300 pt-1 text-base font-bold"><span>{{ __('Total') }}</span><span class="text-brand-700">{{ number_format((float) $order->total, 2) }}</span></div>
