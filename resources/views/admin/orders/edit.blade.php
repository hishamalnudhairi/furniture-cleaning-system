...
@section('content')
    <a href="{{ route('admin.orders.show', $order) }}" class="text-sm text-slate-500 hover:text-slate-700"><span class="ib-flip">←</span> {{ __('Back') }}</a>
    ...
    <form method="POST" action="{{ route('admin.orders.update', $order) }}" class="space-y-5">
        ...
        <section class="rounded-xl bg-white p-5 shadow-sm ring-1 ring-slate-200">
            <div class="mb-4 space-y-1 rounded-lg bg-slate-50 p-4 text-sm">
                ...
                <div class="flex justify-between text-base"><span class="font-semibold">{{ __('Total') }}</span><span id="o-total" class="font-bold text-brand-700">{{ number_format((float) $order->total, 2) }}</span></div>
